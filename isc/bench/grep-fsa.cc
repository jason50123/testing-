...
int main(int argc, const char *argv[]) {
  ...
  data = (char *)config.pattern;
  if (setOpt(config.slet_id, config.nvme, "pattern", data, strlen(data)))
    exit(-__LINE__);
  pr("SetOpt done");
  ...
  auto result = (char *)calloc(1, resSize + 1);
  ...
  pr("result line (%lu): '%s'", resSize, result);
